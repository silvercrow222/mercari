...
describe Product do
  describe '#create' do
    ...
    it "is invalid without a day" do
      ...
      product.valid?
      expect(product.errors[:day]).to include("can't be blank")
    end

    it "is invalid without a shipping" do
      product = build(:product, shipping: nil)
      product.valid?
      expect(product.errors[:shipping]).to include("can't be blank")
    end

    it "is invalid without a fee" do
      product = build(:product, fee: nil)
      product.valid?
      expect(product.errors[:fee]).to include("can't be blank")
    end
